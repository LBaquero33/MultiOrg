...
begin
  ...
  local_time:=(pg_catalog.now() at time zone pg_catalog.coalesce(pref.timezone,'UTC'))::time;
  return pg_catalog.jsonb_build_object('in_app',case when p_required then true else pg_catalog.coalesce(pref.in_app_enabled,true) end,'push',pg_catalog.coalesce(pref.push_enabled,true) and not(case when pref.quiet_hours_start is null then false when pref.quiet_hours_start<pref.quiet_hours_end then local_time>=pref.quiet_hours_start and local_time<pref.quiet_hours_end else local_time>=pref.quiet_hours_start or local_time<pref.quiet_hours_end end),'email_ready',pg_catalog.coalesce(pref.email_ready_enabled,false),'sms_ready',pg_catalog.coalesce(pref.sms_ready_enabled,false),'required',p_required,'timezone',pg_catalog.coalesce(pref.timezone,'UTC'));
end; $$;
...
returns table(user_id uuid,subject_player_id uuid,reason text) language sql stable security definer set search_path='' as $$
  select distinct expanded.user_id,expanded.subject_player_id,expanded.reason from (
    ...
    union all select pm.player_id,pm.player_id,'team_player' from public.sd_player_team_memberships pm where pm.organization_id=p_organization_id and pm.team_id=nullif(p_filter->>'team_id','')::uuid and pm.active and pm.ended_at is null and p_audience_type in ('team','players')
    union all select ca.coach_id,null::uuid,'team_staff' from public.sd_coach_team_assignments ca where ca.organization_id=p_organization_id and ca.team_id=nullif(p_filter->>'team_id','')::uuid and ca.active and ca.ended_at is null and p_audience_type in ('team','team_staff')
    union all select link.parent_id,pm.player_id,'linked_parent' from public.sd_player_team_memberships pm join public.sd_parent_child_links link on link.org_id=pm.organization_id and link.child_id=pm.player_id where pm.organization_id=p_organization_id and pm.team_id=nullif(p_filter->>'team_id','')::uuid and pm.active and pm.ended_at is null and p_audience_type in ('team','parents','households')
...
declare announcement public.sd_announcements%rowtype; recipients uuid[]; created_count integer:=0; replay_target uuid; filter jsonb:=pg_catalog.coalesce(p_payload->'audience_filter','{}'::jsonb);
begin
  ...
  insert into public.sd_announcements(organization_id,season_id,team_id,event_id,title,body,audience_type,audience_filter,priority,visibility,acknowledgment_required,publish_at,expires_at,status,created_by,updated_by)
    values(p_organization_id,nullif(p_payload->>'season_id','')::uuid,nullif(p_payload->>'team_id','')::uuid,nullif(p_payload->>'event_id','')::uuid,pg_catalog.btrim(p_payload->>'title'),pg_catalog.btrim(p_payload->>'body'),p_payload->>'audience_type',filter,pg_catalog.coalesce(nullif(p_payload->>'priority',''),'normal'),pg_catalog.coalesce(nullif(p_payload->>'visibility',''),'audience'),pg_catalog.coalesce((p_payload->>'acknowledgment_required')::boolean,false),pg_catalog.coalesce((p_payload->>'publish_at')::timestamptz,pg_catalog.now()),(p_payload->>'expires_at')::timestamptz,'published',p_actor_id,p_actor_id) returning * into announcement;
  ...
  select pg_catalog.coalesce(pg_catalog.array_agg(recipient_user_id),'{}'::uuid[]) into recipients from public.sd_announcement_recipients where announcement_id=announcement.id;
...
begin
  ...
  if pref.id is null then insert into public.sd_notification_preferences(user_id,organization_id,team_id,subject_player_id,category,in_app_enabled,push_enabled,email_ready_enabled,sms_ready_enabled,quiet_hours_start,quiet_hours_end,timezone) values(p_actor_id,p_organization_id,p_team_id,p_subject_player_id,p_category,pg_catalog.coalesce((p_payload->>'in_app_enabled')::boolean,true),pg_catalog.coalesce((p_payload->>'push_enabled')::boolean,true),pg_catalog.coalesce((p_payload->>'email_ready_enabled')::boolean,false),pg_catalog.coalesce((p_payload->>'sms_ready_enabled')::boolean,false),(p_payload->>'quiet_hours_start')::time,(p_payload->>'quiet_hours_end')::time,pg_catalog.coalesce(nullif(p_payload->>'timezone',''),'UTC')) returning * into pref;
  else if p_expected_version is null or pref.version<>p_expected_version then raise exception using errcode='P0001',message='stale_notification_preference'; end if; update public.sd_notification_preferences set in_app_enabled=pg_catalog.coalesce((p_payload->>'in_app_enabled')::boolean,in_app_enabled),push_enabled=pg_catalog.coalesce((p_payload->>'push_enabled')::boolean,push_enabled),email_ready_enabled=pg_catalog.coalesce((p_payload->>'email_ready_enabled')::boolean,email_ready_enabled),sms_ready_enabled=pg_catalog.coalesce((p_payload->>'sms_ready_enabled')::boolean,sms_ready_enabled),quiet_hours_start=case when p_payload?'quiet_hours_start' then (p_payload->>'quiet_hours_start')::time else quiet_hours_start end,quiet_hours_end=case when p_payload?'quiet_hours_end' then (p_payload->>'quiet_hours_end')::time else quiet_hours_end end,timezone=pg_catalog.coalesce(nullif(p_payload->>'timezone',''),timezone),version=version+1,updated_at=pg_catalog.now() where id=pref.id returning * into pref; end if;
...
begin
  insert into public.sd_notification_deliveries(notification_id,device_id,channel,status,next_attempt_at)
    ...
    where pg_catalog.coalesce((n.metadata->>'push_suppressed')::boolean,false)=false on conflict(notification_id,device_id,channel) do nothing;
  get diagnostics created_count=row_count;
  if created_count>0 and pg_catalog.coalesce(pg_catalog.current_setting('app.notification_delivery_enabled',true),'false')='true' then perform public.sd_request_notification_delivery_worker(); end if;
  return null;
end $$;
...
begin
  ...
  title:=pg_catalog.coalesce(nullif(intent.payload->>'title',''),pg_catalog.initcap(pg_catalog.replace(intent.intent_type,'_',' ')));
  body:=pg_catalog.coalesce(nullif(intent.payload->>'body',''),'Open Home Plate for the latest event details.');
  for recipient in
...
begin
  ...
  select pg_catalog.coalesce(message_edit_window_minutes,15) into edit_window from public.sd_communication_policies where organization_id=p_organization_id;
  edit_window:=pg_catalog.coalesce(edit_window,15);
...
begin
  ...
  if is_admin and message.sender_id<>p_actor_id and pg_catalog.btrim(pg_catalog.coalesce(p_reason,''))='' then raise exception using errcode='22023',message='moderation_reason_required'; end if;
...
begin
  ...
  if sender_role='player' and has_staff and not pg_catalog.coalesce(policy.player_to_coach_allowed,true) then raise exception using errcode='42501',message='player_to_coach_messaging_disabled'; end if;
  if sender_role='parent' and has_staff and not pg_catalog.coalesce(policy.parent_to_coach_allowed,true) then raise exception using errcode='42501',message='parent_to_coach_messaging_disabled'; end if;
