...
begin
  ...
  if p_action in ('initialize_game_plan','duplicate_prior_game_plan') then
    ...
    if p_action='update_game_plan' then
      ...
      response:=pg_catalog.jsonb_build_object('plan',pg_catalog.to_jsonb(plan),'batting_order',(select coalesce(pg_catalog.jsonb_agg(pg_catalog.to_jsonb(b) order by batting_slot),'[]'::jsonb) from public.sd_game_batting_entries b where b.game_plan_id=plan.id and b.active));
