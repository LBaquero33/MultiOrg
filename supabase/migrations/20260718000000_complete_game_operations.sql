...
create table if not exists public.sd_game_plan_eligibility (
  ...
  check (status not in ('coach_excluded','rostered_not_dressing') or pg_catalog.char_length(pg_catalog.btrim(coalesce(exclusion_reason,''))) > 0)
);
...
returns jsonb language sql stable security definer set search_path='' as $$
  select pg_catalog.jsonb_build_object(
    ...
    'eligibility', (select coalesce(pg_catalog.jsonb_agg(pg_catalog.to_jsonb(x) order by x.player_id), '[]'::jsonb) from public.sd_game_plan_eligibility x where x.game_plan_id=plan.id),
    'batting_order', (select coalesce(pg_catalog.jsonb_agg(pg_catalog.to_jsonb(x) order by x.batting_slot nulls last, x.created_at), '[]'::jsonb) from public.sd_game_batting_entries x where x.game_plan_id=plan.id and x.active),
    'defense', (select coalesce(pg_catalog.jsonb_agg(pg_catalog.to_jsonb(x) order by x.inning_number, x.position_code), '[]'::jsonb) from public.sd_game_defensive_assignments x where x.game_plan_id=plan.id and x.active),
    'pitcher_catcher', (select coalesce(pg_catalog.jsonb_agg(pg_catalog.to_jsonb(x) order by x.role_type, x.sequence_index), '[]'::jsonb) from public.sd_game_pitcher_catcher_plans x where x.game_plan_id=plan.id and x.archived_at is null),
    'staff', (select coalesce(pg_catalog.jsonb_agg(pg_catalog.to_jsonb(x) order by x.responsibility_code), '[]'::jsonb) from public.sd_game_staff_assignments x where x.game_plan_id=plan.id and x.active),
    'result', pg_catalog.to_jsonb(result),
    'recaps', (select coalesce(pg_catalog.jsonb_agg(pg_catalog.to_jsonb(x) order by x.created_at), '[]'::jsonb) from public.sd_game_recaps x where x.game_plan_id=plan.id)
...
begin
  ...
  if exists(select 1 from pg_catalog.jsonb_array_elements(public.sd_team_event_conflicts(plan.organization_id,plan.team_id,(select start_at from public.sd_team_events where id=plan.event_id),(select end_at from public.sd_team_events where id=plan.event_id),null,(select coalesce(pg_catalog.array_agg(staff_user_id),'{}'::uuid[]) from public.sd_game_staff_assignments where game_plan_id=plan.id and active),plan.event_id)) conflict where conflict->>'type'='coach') then warnings:=warnings||pg_catalog.jsonb_build_array(pg_catalog.jsonb_build_object('code','coach_assignment_conflict','severity','readiness_warning')); end if;
...
returns text[] language sql stable security definer set search_path='' as $$
  ...
  ) select coalesce(pg_catalog.array_agg(capability order by capability),'{}'::text[]) from (select capability from base union select capability from practice union select capability from game) all_caps;
$$;
...
declare evt public.sd_team_events%rowtype; plan public.sd_game_plans%rowtype;
  ...
  fingerprint text; v_response jsonb; previous jsonb; reason text; audit_action text:=p_action;
...
begin
  ...
  if p_action in ('initialize_game_plan','duplicate_prior_game_plan') then
    if plan.id is null then
      insert into public.sd_game_plans(organization_id,season_id,team_id,event_id,event_operation_id,title,lineup_mode,created_by,updated_by)
        values(p_organization_id,evt.season_id,evt.team_id,evt.id,(select id from public.sd_event_operations where event_id=evt.id),coalesce(nullif(pg_catalog.btrim(p_payload->>'title'),''),evt.title||' Game Plan'),coalesce(nullif(p_payload->>'lineup_mode',''),'custom'),p_actor_id,p_actor_id) returning * into plan;
    ...
    v_response:=pg_catalog.jsonb_build_object('plan',pg_catalog.to_jsonb(plan),'initialized',plan.version=1,'duplicated',p_action='duplicate_prior_game_plan');
    ...
    if p_action='duplicate_rule_profile' then
      target:=coalesce(nullif(p_payload->>'source_rule_profile_id',''),nullif(p_payload->>'rule_profile_id',''))::uuid;
    ...
    if p_action='update_game_plan' then
      ...
      if p_action<>'clear_batting_order' then
        slot:=0;
        for item in select value from pg_catalog.jsonb_array_elements(coalesce(p_payload->'entries','[]'::jsonb)) loop
          slot:=slot+1;
          insert into public.sd_game_batting_entries(game_plan_id,organization_id,player_id,batting_slot,offensive_role,role_label,source,notes,created_by,updated_by)
            values(plan.id,p_organization_id,(item->>'player_id')::uuid,coalesce((item->>'batting_slot')::integer,slot),coalesce(nullif(item->>'offensive_role',''),'hitter'),nullif(item->>'role_label',''),case when p_action='initialize_bat_entire_roster' then 'roster' else 'manual' end,nullif(item->>'notes',''),p_actor_id,p_actor_id);
        end loop;
      ...
      v_response:=pg_catalog.jsonb_build_object('plan',pg_catalog.to_jsonb(plan),'batting_order',(select coalesce(pg_catalog.jsonb_agg(pg_catalog.to_jsonb(b) order by batting_slot),'[]'::jsonb) from public.sd_game_batting_entries b where b.game_plan_id=plan.id and b.active));
      ...
      insert into public.sd_game_batting_entries(game_plan_id,organization_id,player_id,batting_slot,offensive_role,role_label,starter,eligible,source,notes,created_by,updated_by)
        values(plan.id,p_organization_id,(p_payload->>'player_id')::uuid,(p_payload->>'batting_slot')::integer,coalesce(nullif(p_payload->>'offensive_role',''),'hitter'),nullif(p_payload->>'role_label',''),coalesce((p_payload->>'starter')::boolean,true),coalesce((p_payload->>'eligible')::boolean,true),case when plan.status='active' then 'active_adjustment' else 'manual' end,nullif(p_payload->>'notes',''),p_actor_id,p_actor_id) returning id into target;
      update public.sd_game_plans set version=version+1,updated_by=p_actor_id,updated_at=pg_catalog.now() where id=plan.id returning * into plan; v_response:=pg_catalog.jsonb_build_object('plan',pg_catalog.to_jsonb(plan),'batting_entry_id',target);
      ...
      update public.sd_game_plans set version=version+1,updated_by=p_actor_id,updated_at=pg_catalog.now() where id=plan.id returning * into plan; v_response:=pg_catalog.jsonb_build_object('plan',pg_catalog.to_jsonb(plan),'removed_entry_id',target);
      ...
      update public.sd_game_plans set version=version+1,updated_by=p_actor_id,updated_at=pg_catalog.now() where id=plan.id returning * into plan; v_response:=pg_catalog.jsonb_build_object('plan',pg_catalog.to_jsonb(plan));
      ...
      update public.sd_game_plans set version=version+1,updated_by=p_actor_id,updated_at=pg_catalog.now() where id=plan.id returning * into plan; v_response:=pg_catalog.jsonb_build_object('plan',pg_catalog.to_jsonb(plan),'player_id',target);
      ...
      insert into public.sd_game_defensive_assignments(game_plan_id,organization_id,player_id,inning_number,inning_half,position_code,position_label,assignment_type,starter,notes,created_by,updated_by)
        values(plan.id,p_organization_id,(p_payload->>'player_id')::uuid,coalesce((p_payload->>'inning_number')::integer,0),coalesce(nullif(p_payload->>'inning_half',''),'defense'),pg_catalog.upper(p_payload->>'position_code'),nullif(p_payload->>'position_label',''),coalesce(nullif(p_payload->>'assignment_type',''),case when coalesce((p_payload->>'inning_number')::integer,0)=0 then 'starter' else 'planned' end),coalesce((p_payload->>'starter')::boolean,coalesce((p_payload->>'inning_number')::integer,0)=0),nullif(p_payload->>'notes',''),p_actor_id,p_actor_id) returning id into target;
      update public.sd_game_plans set version=version+1,updated_by=p_actor_id,updated_at=pg_catalog.now() where id=plan.id returning * into plan; v_response:=pg_catalog.jsonb_build_object('plan',pg_catalog.to_jsonb(plan),'defensive_assignment_id',target);
      ...
      update public.sd_game_plans set version=version+1,updated_by=p_actor_id,updated_at=pg_catalog.now() where id=plan.id returning * into plan; v_response:=pg_catalog.jsonb_build_object('plan',pg_catalog.to_jsonb(plan),'removed_assignment_id',target);
      ...
      if p_action='update_pitcher_catcher_plan' then
        ...
        update public.sd_game_pitcher_catcher_plans set player_id=case when p_payload?'player_id' then (p_payload->>'player_id')::uuid else player_id end,role_type=coalesce(nullif(p_payload->>'role_type',''),role_type),sequence_index=case when p_payload?'sequence_index' then (p_payload->>'sequence_index')::integer else sequence_index end,planned_start_inning=case when p_payload?'planned_start_inning' then (p_payload->>'planned_start_inning')::integer else planned_start_inning end,planned_end_inning=case when p_payload?'planned_end_inning' then (p_payload->>'planned_end_inning')::integer else planned_end_inning end,manual_pitch_limit=case when p_payload?'manual_pitch_limit' then (p_payload->>'manual_pitch_limit')::integer else manual_pitch_limit end,pairing_player_id=case when p_payload?'pairing_player_id' then nullif(p_payload->>'pairing_player_id','')::uuid else pairing_player_id end,notes=case when p_payload?'notes' then nullif(p_payload->>'notes','') else notes end,status=coalesce(nullif(p_payload->>'status',''),status),version=version+1,updated_by=p_actor_id,updated_at=pg_catalog.now() where id=target and game_plan_id=plan.id and version=expected and archived_at is null;
        if not found then raise exception using errcode='P0001',message='stale_or_missing_pitcher_catcher_plan'; end if;
      else
        insert into public.sd_game_pitcher_catcher_plans(game_plan_id,organization_id,player_id,role_type,sequence_index,planned_start_inning,planned_end_inning,manual_pitch_limit,pairing_player_id,notes,status,created_by,updated_by)
          values(plan.id,p_organization_id,(p_payload->>'player_id')::uuid,case p_action when 'assign_starting_pitcher' then 'starting_pitcher' when 'add_relief_pitcher' then 'relief_pitcher' when 'assign_starting_catcher' then 'starting_catcher' when 'assign_backup_catcher' then 'backup_catcher' else coalesce(nullif(p_payload->>'role_type',''),'relief_pitcher') end,coalesce((p_payload->>'sequence_index')::integer,0),(p_payload->>'planned_start_inning')::integer,(p_payload->>'planned_end_inning')::integer,(p_payload->>'manual_pitch_limit')::integer,nullif(p_payload->>'pairing_player_id','')::uuid,nullif(p_payload->>'notes',''),coalesce(nullif(p_payload->>'status',''),'planned'),p_actor_id,p_actor_id) returning id into target;
      end if;
      update public.sd_game_plans set version=version+1,updated_by=p_actor_id,updated_at=pg_catalog.now() where id=plan.id returning * into plan; v_response:=pg_catalog.jsonb_build_object('plan',pg_catalog.to_jsonb(plan),'pitcher_catcher_id',target);
      ...
      update public.sd_game_plans set version=version+1,updated_by=p_actor_id,updated_at=pg_catalog.now() where id=plan.id returning * into plan; v_response:=pg_catalog.jsonb_build_object('plan',pg_catalog.to_jsonb(plan),'staff_assignment_id',target);
      ...
      v_response:=pg_catalog.jsonb_build_object('plan',pg_catalog.to_jsonb(plan),'snapshot',pg_catalog.to_jsonb(snap),'validation',public.sd_validate_game_plan(plan.id));
      ...
      v_response:=pg_catalog.jsonb_build_object('plan',pg_catalog.to_jsonb(plan),'started_snapshot',pg_catalog.to_jsonb(snap));
      ...
      insert into public.sd_game_active_adjustments(game_plan_id,started_snapshot_id,adjustment_type,target_id,previous_value,new_value,reason,actor_id)
        values(plan.id,started_id,case p_action when 'apply_active_lineup_adjustment' then 'lineup' when 'apply_active_defense_adjustment' then 'defense' when 'apply_active_pitcher_adjustment' then 'pitcher_catcher' else 'eligibility' end,nullif(p_payload->>'target_id','')::uuid,p_payload->'previous_value',coalesce(p_payload->'new_value','{}'::jsonb),reason,p_actor_id) returning id into target;
      update public.sd_game_plans set version=version+1,updated_by=p_actor_id,updated_at=pg_catalog.now() where id=plan.id returning * into plan; v_response:=pg_catalog.jsonb_build_object('plan',pg_catalog.to_jsonb(plan),'adjustment_id',target);
      ...
      insert into public.sd_game_results(game_plan_id,organization_id,team_score,opponent_score,outcome,innings_played,ended_early,end_reason,result_status,result_notes,recorded_by,recorded_at)
        values(plan.id,p_organization_id,(p_payload->>'team_score')::integer,(p_payload->>'opponent_score')::integer,coalesce(nullif(p_payload->>'outcome',''),'unknown'),(p_payload->>'innings_played')::integer,coalesce((p_payload->>'ended_early')::boolean,false),nullif(p_payload->>'end_reason',''),case when result_row.id is null then 'recorded' else 'corrected' end,nullif(p_payload->>'result_notes',''),p_actor_id,pg_catalog.now())
      ...
      v_response:=pg_catalog.jsonb_build_object('plan',pg_catalog.to_jsonb(plan),'result',pg_catalog.to_jsonb(result_row));
      ...
      insert into public.sd_game_recaps(game_plan_id,organization_id,visibility,subject_player_id,body,follow_up_items,published_at,created_by,updated_by)
        values(plan.id,p_organization_id,p_payload->>'visibility',nullif(p_payload->>'subject_player_id','')::uuid,pg_catalog.btrim(p_payload->>'body'),coalesce(array(select pg_catalog.jsonb_array_elements_text(p_payload->'follow_up_items')),'{}'::text[]),case when (p_payload->>'publish')::boolean then pg_catalog.now() else null end,p_actor_id,p_actor_id) returning id into target;
      update public.sd_game_plans set version=version+1,updated_by=p_actor_id,updated_at=pg_catalog.now() where id=plan.id returning * into plan;
      v_response:=pg_catalog.jsonb_build_object('plan',pg_catalog.to_jsonb(plan),'recap_id',target);
      ...
      insert into public.sd_game_plan_snapshots(game_plan_id,organization_id,event_operation_id,snapshot_type,plan_version,snapshot,reason,created_by)
        values(plan.id,p_organization_id,plan.event_operation_id,'completed',plan.version,public.sd_game_plan_snapshot(plan.id)||pg_catalog.jsonb_build_object('adjustments',(select coalesce(pg_catalog.jsonb_agg(pg_catalog.to_jsonb(a) order by created_at),'[]'::jsonb) from public.sd_game_active_adjustments a where a.game_plan_id=plan.id)),nullif(p_payload->>'completion_notes',''),p_actor_id)
        on conflict(game_plan_id,snapshot_type,plan_version) do update set snapshot=excluded.snapshot,reason=excluded.reason returning * into snap;
      ...
      v_response:=pg_catalog.jsonb_build_object('plan',pg_catalog.to_jsonb(plan),'completion_snapshot',pg_catalog.to_jsonb(snap));
      ...
      update public.sd_game_plans set status='active',version=version+1,updated_by=p_actor_id,updated_at=pg_catalog.now() where id=plan.id returning * into plan; v_response:=pg_catalog.jsonb_build_object('plan',pg_catalog.to_jsonb(plan));
  ...
  if plan.id is not null then
    ...
    insert into public.sd_game_plan_audit_logs(organization_id,season_id,team_id,event_id,game_plan_id,actor_id,action,request_id,target_id,previous_value,new_value,reason)
      values(p_organization_id,evt.season_id,evt.team_id,evt.id,plan.id,p_actor_id,audit_action,p_request_id,target,previous,v_response,reason);
    ...
    elsif p_action='add_game_recap' and coalesce((p_payload->>'publish')::boolean,false) then
    ...
    insert into public.sd_game_plan_audit_logs(organization_id,season_id,team_id,event_id,game_plan_id,actor_id,action,request_id,target_id,previous_value,new_value,reason)
      values(p_organization_id,evt.season_id,evt.team_id,evt.id,null,p_actor_id,audit_action,p_request_id,target,previous,v_response,reason);
  end if;
  update public.sd_game_plan_mutations set status='completed',response=v_response,completed_at=pg_catalog.now() where organization_id=p_organization_id and request_id=p_request_id;
  return v_response;
