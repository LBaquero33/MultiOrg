...
declare from_at timestamptz:=coalesce((p_filters->>'from')::timestamptz,'-infinity'::timestamptz); to_at timestamptz:=coalesce((p_filters->>'to')::timestamptz,pg_catalog.now()); season_filter uuid:=nullif(p_filters->>'season_id','')::uuid; team_filter uuid:=nullif(p_filters->>'team_id','')::uuid; result jsonb;
begin
  ...
  with financial as (
    select
      coalesce((select sum(i.total_cents) from public.sd_invoices i where i.org_id=p_organization_id and i.financial_layer='organization_customer' and i.status<>'draft' and i.created_at between from_at and to_at and (season_filter is null or i.season_id=season_filter) and (team_filter is null or i.team_id=team_filter)),0) gross_invoiced,
      coalesce((select sum(p.amount_cents) from public.sd_payments p where p.org_id=p_organization_id and p.financial_layer='organization_customer' and p.status in ('succeeded','paid') and coalesce(p.received_at,p.paid_at,p.created_at) between from_at and to_at),0) collected,
      coalesce((select sum(i.amount_remaining_cents) from public.sd_invoices i where i.org_id=p_organization_id and i.financial_layer='organization_customer' and i.status in ('issued','partially_paid','overdue')),0) receivable,
      coalesce((select sum(i.amount_remaining_cents) from public.sd_invoices i where i.org_id=p_organization_id and i.financial_layer='organization_customer' and i.status in ('issued','partially_paid','overdue') and i.due_date<current_date),0) overdue,
      coalesce((select sum(r.amount_cents) from public.sd_refunds r where r.org_id=p_organization_id and r.financial_layer='organization_customer' and r.status in ('succeeded','confirmed') and coalesce(r.confirmed_at,r.created_at) between from_at and to_at),0) refunds,
      coalesce((select sum(e.amount_cents) from public.sd_expenses e where e.org_id=p_organization_id and e.financial_layer='organization_expense' and e.expense_date between from_at::date and to_at::date and (season_filter is null or e.season_id=season_filter) and (team_filter is null or e.team_id=team_filter)),0) expenses
  ), registration as (
    select pg_catalog.count(*) total,pg_catalog.count(*) filter(where a.state='draft') drafts,pg_catalog.count(*) filter(where a.state in ('submitted','under_review','action_required')) submitted,pg_catalog.count(*) filter(where a.state='approved') approved,pg_catalog.count(*) filter(where a.state='waitlisted') waitlisted,pg_catalog.count(*) filter(where a.state='declined') declined,pg_catalog.count(*) filter(where a.state in ('assigned','completed')) assigned,coalesce(sum(a.balance_cents),0) balance
...
create or replace function public.sd_resolve_organization_capabilities(target_organization uuid,target_actor uuid default auth.uid()) returns text[] language sql stable security definer set search_path='' as $$
  ...
  ) select coalesce(pg_catalog.array_agg(capability order by capability),'{}'::text[]) from capabilities;
$$;
...
declare organization_id uuid; intent_type text; payload jsonb; v_deduplication_key text; application_id uuid; customer_account_id uuid; invoice_id uuid; recipient_id uuid; subject_id uuid; preference jsonb; v_category text; source_name text; route text; title text; body text; required boolean:=false; notification_id uuid; delivered integer:=0; suppressed integer:=0;
begin
  if p_source_type='registration' then
    select i.organization_id,i.intent_type,i.payload,i.deduplication_key,i.application_id into organization_id,intent_type,payload,v_deduplication_key,application_id from public.sd_registration_notification_intents i where i.id=p_source_id and i.consumed_at is null for update;
    select coalesce(a.guardian_user_id,a.applicant_user_id),a.player_user_id into recipient_id,subject_id from public.sd_registration_applications a where a.id=application_id;
    v_category:=case when intent_type='registration_balance_due' then 'payment_notice' else 'registration' end; source_name:='registration'; route:='registration';
  elsif p_source_type='finance' then
    select i.org_id,i.intent_type,i.payload,i.deduplication_key,i.customer_account_id,i.invoice_id into organization_id,intent_type,payload,v_deduplication_key,customer_account_id,invoice_id from public.sd_financial_notification_intents i where i.id=p_source_id and i.consumed_at is null for update;
    select coalesce(a.responsible_user_id,a.user_id),cap.player_id into recipient_id,subject_id from public.sd_customer_accounts a left join lateral(select player_id from public.sd_customer_account_players where account_id=a.id order by created_at limit 1) cap on true where a.id=customer_account_id;
    ...
    v_category:='payment_notice'; source_name:='organization_finance'; route:='finance';
  ...
  title:=pg_catalog.initcap(pg_catalog.replace(intent_type,'_',' ')); body:=coalesce(nullif(payload->>'body',''),'Open Home Plate for details.');
  preference:=public.sd_effective_notification_preference(recipient_id,organization_id,null,subject_id,case v_category when 'payment_notice' then 'payments' else 'registration' end,required);
  if not p_dry_run then
    if (preference->>'in_app')::boolean or (preference->>'push')::boolean then
      insert into public.sd_notifications(org_id,recipient_user_id,category,title,body,related_entity_type,related_entity_id,action_route,action_payload,deduplication_key,created_by,source,metadata,archived_at)
        values(organization_id,recipient_id,v_category,title,body,p_source_type,p_source_id::text,route,payload,v_deduplication_key,p_actor_id,source_name,pg_catalog.jsonb_build_object('intent_id',p_source_id,'push_suppressed',not((preference->>'push')::boolean)),case when (preference->>'in_app')::boolean then null else pg_catalog.now() end)
