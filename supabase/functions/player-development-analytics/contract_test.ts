...
Deno.test("analytics cache contract matches the baseball-native response", () => {
  assertEquals(ANALYTICS_SCHEMA_VERSION, 2);
  assertEquals(ANALYTICS_MODEL_VERSION, "homeplate-r-analytics.v2");
});
...
Deno.test("analytics player scope denies unrelated roles and teams", () => {
  ...
  assert(
    canAccessPlayer("owner", actor, player, new Set(), new Set(), new Set()),
  );
  assert(
    canAccessPlayer("player", player, player, new Set(), new Set(), new Set()),
  );
  assert(
    canAccessPlayer(
      "parent",
