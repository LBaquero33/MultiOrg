...
struct OrganizationSetupTests {
  ...
  func invitationSecurityContract() throws {
    ...
    let appState = try String(contentsOf: root.appendingPathComponent("MultiOrg/Core/AppState.swift"))
  ...
  func inviteUnavailablePresentation() throws {
    let root = URL(fileURLWithPath: #filePath).deletingLastPathComponent().deletingLastPathComponent()
    let appState = try String(contentsOf: root.appendingPathComponent("MultiOrg/Core/AppState.swift"))
    let login = try String(contentsOf: root.appendingPathComponent("MultiOrg/Features/Login/LoginView.swift"))
  ...
  func scheduleRegressionContract() throws {
    ...
    let schedule = try String(contentsOf: root.appendingPathComponent("MultiOrg/Features/Coach/CoachTeamScheduleView.swift"))
